...
class Channel(object):
    ...
    def get_jobs_to_run(self, now, jobs_to_get=None):
        ...
        _deferred = SafeSet()
        jobs_to_get = jobs_to_get_orig  # reset the counter of jobs to return
        found_job = True
        while (not self.capacity or len(self._running) < self.capacity
               or (jobs_to_get and found_job)):
            found_job = False
            ...
            if not job:
                ...
                if (jobs_to_get is None and self.children):
                    overcapacity = (
                        self.capacity - len(self._running) -
                        sum(max(child.capacity - len(child._running), 0)
                            for child in self.children.values()))
                    if overcapacity > 0:
                        for job in self.get_jobs_to_run(
                                now, jobs_to_get=overcapacity):
                            yield job
                return
